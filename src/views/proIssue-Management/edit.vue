<template>
  <div class="ediProIssue">
    <div class="ediProIssue-header">编辑项目问题</div>
    <div class="ediProIssue-form">
      <DataForm 
...
import { updateProIssueInfo } from '@/api/proIssue.js';
...
export default {
  data () {
    return {
      ...
      formInfo: {
        ref: 'addProIssue',
        labelPosition:'left',
        data: JSON.parse(sessionStorage.getItem('editProIssue')),
        fieldList : [
          {label: '项目编号', value: 'projectNumber', type: 'input', disabled: true, required: true},
  ...
  methods: {
    ...
    async handleClick() {
      let params = {
        id: JSON.parse(sessionStorage.getItem('editProIssue')).id,
      ...
      let res = await updateProIssueInfo(params);
