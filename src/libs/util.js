...
export const getTagNavListFromLocalstorage = () => {
  ...
  if (allmenuName && allmenuName.length == 0) {
    ...
    for (let index = 0; index < tagList.length; index++) {
      if (
        tagList[index].name !== "index" &&
        !tagList[index].name.includes(allmenuName)
      ) {
      ...
      if (!hasChild(item)) {
        ...
        if (item.target === "_blank") {
          ...
          if (item.scheme === "/") {
            // 内部组件
            router.component = resolve => {
              require([`_v/module/${path}.vue`], resolve);
            };
          } else {
            // 传递iframe路径参数
...
export const dateFormatter = (date) => {
  let tempTime = new Date(date).getTime() + 8*60*60*1000;
