<template>
  <div class="addRole">
    <div class="addRole-header">编辑角色</div>
...
export default {
  data () {
    return {
      ...
      formInfo: {
        ref: 'editRole',
        labelPosition:'left',
        data: JSON.parse(sessionStorage.getItem('editRole')),
        fieldList : [
          {label: '角色名称', value: 'roleName', type: 'input', disabled: true},
          {label: '角色标识', value: 'roleCode', type: 'input', disabled: true},
  ...
  methods: {
    ...
    async handleClick() {
      let params = {
        id: JSON.parse(sessionStorage.getItem('editRole')).id,
        status: this.formInfo.data.status,
      }
      // formInfo.ref.validate(async valid => {
      //   if(valid) {
          let res = await updateRoleInfo(params);
