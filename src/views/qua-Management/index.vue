...
import { getQuaList, delQuaInfo } from "@/api/qualification.js";
export default {
  data () {
    return {
      getQuaList,
      ...
      filterInfo: {
        ...
        list: [
          { type: "input", label: "资质名称:", value: "certificateName" },
          { type: "input", label: "证书编号:", value: "certificateNumber" },
          { type: "select", label: "用户名证书类型:", value: "certificateType", list: 'flagList'},
          {
            type: "button",
            name: "查询",
            btType: "primary",
      ...
      tableInfo: {
        ...
        data: [],
        fieldList: [
          { label: '资质名称', value: 'certificateName' },
          { label: '证书类型', value: 'certiType' },
          { label: '证书编号', value: 'certificateNumber' },
          { label: '获证日期', value: 'certificateDate' },
          { label: '有效截止日期', value: 'effectiveDate' },
          { label: '发证机构', value: 'organization' },
        ],
        handle: {
          fixed: "right",
          label: "操作",
  ...
  methods: {
    ...
    handleClick(event, data) {
      ...
      switch (event) {
        ...
        case "create":
          ...
          break;
        case "edit":
          sessionStorage.editQua = JSON.stringify(data);
          this.$router.push({name:'editQua'});
          break;
        // 查看和编辑
        case "view":
        ...
        case "delete":
          ...
          this.$handleAPI(event, delQuaInfo, params).then(res => {
            if (res.success) {
              this.tableInfo.refresh = Math.random();
            }
          });
      ...
      switch (event) {
        ...
        case "list":
          ...
          data.forEach(item => {
            ...
            item.certiType = tempNumber[item.certificateType];
          });
          break;
        case "tableCheck":
    ...
    toDel() {
      if (this.currentSelectlist.length === 0) {
        ...
        console.log(params)
        this.$handleAPI("delete", delQuaInfo, params).then(
          res => {
            if (res.success) {
              tableInfo.refresh = Math.random();
            }
