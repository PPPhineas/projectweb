...
import { getProIssueList, delProIssueInfo } from "@/api/proIssue.js";
export default {
  data () {
    return {
      getProIssueList,
      ...
      tableInfo: {
        ...
        fieldList: [
          ...
          {label: '发现日期', value: 'presentation', minWidth: 160},
          {label: '问题描述', value: 'descr'},
          {label: '解决方案', value: 'solution'},
          {label: '解决时间', value: 'solutionTime', minWidth: 160},
          {label: '负责人', value: 'principal'},
          {label: '验证人', value: 'identifier'},
  ...
  methods: {
    ...
    handleClick(event, data) {
      ...
      switch (event) {
        ...
        case "create":
          ...
          break;
        case "edit":
          sessionStorage.editProIssue = JSON.stringify(data);
          this.$router.push({name:'editProIssue'});
          break;
        // 查看和编辑
        case "view":
        ...
        case "delete":
          ...
          this.$handleAPI(event, delProIssueInfo, params).then(res => {
            if (res.success) {
              this.tableInfo.refresh = Math.random();
            }
          });
    ...
    toAdd() {
      this.$router.push({ name: "addProjectIssue" });
    },
    toDel() {
      if (this.currentSelectlist.length === 0) {
        ...
        console.log(params)
        this.$handleAPI("delete", delProIssueInfo, params).then(
          res => {
            if (res.success) {
              this.tableInfo.refresh = Math.random();
            }
