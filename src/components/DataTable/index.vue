<template>
  <div class="data-table" :class="className">
    ...
    >
      ...
      >
        <template v-slot="scope">
          ...
          <span
            v-else
          >{{ $fn.getDataName({dataList: listTypeInfo[item.list], value: 'value', label: 'key', data: scope.row[item.value]}) || '-' }}</span>
        </template>
      </el-table-column>
      <el-table-column
        v-if="handle"
      ...
      return new Promise((resolve, reject) => {
        ...
        api(this.handleParams())
          .then(res => {
            ...
            if (res.success) {
              // 使外面可以访问到表格数据
              const arr = res.body.records;
              this.$emit("update:data", arr);
              if (this.pager) {
                this.listInfo.total = res.body.total;
                // this.listInfo.query.current = res.body.current - 0;
                // this.listInfo.query.size = res.body.pages - 0;
              }
              // 设置当前选中项
